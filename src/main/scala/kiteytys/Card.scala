...
object Card {
  ...
  case class Collection(cards: Map[Code, Card]) extends AnyVal {
    def get(code: Code): Option[Card] = cards.get(code.toUpperCase)
    def apply(code: Code): Card = cards.getOrElse(code.toUpperCase, Card(code = code, subject = code, sentence = ""))
  ...
  def collectionFromIterable(iterable: Iterable[Card]): Collection = {
    val cards = iterable
      .map(card => card.copy(card.code.toUpperCase))
      .groupBy(_.code)
      .mapValues(_.head)
    Collection(cards)
  }
